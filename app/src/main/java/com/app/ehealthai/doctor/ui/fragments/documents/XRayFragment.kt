...
import kotlinx.android.synthetic.main.fragment_xray_fragment.view.*
...
class XRayFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_xray_fragment, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        if (PatientDocumentsActivity.xRayList!!.isEmpty()) {
            view.xray_no_docs_image.visibility = View.VISIBLE
        } else {
            view.xray_no_docs_image.visibility = View.GONE
            view.x_ray_recycler_view.layoutManager = LinearLayoutManager(context)
            userDocumentsRecyclerViewAdapter =
                UserDocumentsRecyclerViewAdapter(
                    context!!,
                    PatientDocumentsActivity.xRayList!!,
                    openDialog = {
                        val newDialog = ViewImageDialog(it)
                        newDialog.show(fragmentManager!!, "open Image")
                        fragmentManager!!.executePendingTransactions()
                    })
            view.x_ray_recycler_view.adapter = userDocumentsRecyclerViewAdapter
