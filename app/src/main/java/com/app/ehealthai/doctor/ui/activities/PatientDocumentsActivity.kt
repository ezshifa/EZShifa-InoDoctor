...
import com.app.ehealthaidoctor.ui.fragments.documents.DocNotesFragment
...
import kotlinx.android.synthetic.main.activity_patient_documents.*
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class PatientDocumentsActivity : AppCompatActivity(), LabReportsFragment.OnFragmentInteractionListener  {
    ...
    fun init() {
        user_docs_back_btn.setOnClickListener { onBackPressed() }
        patientId = intent.getStringExtra("patientId")
        val userData=SharedPrefs.getUserData(this@PatientDocumentsActivity)
        val username=userData!!.username
        val usertype="DOCTOR"
        documents_progress_bar.visibility = View.VISIBLE
        ApiUtils.getAPIService(this).getPatientDocuments(patientId!!, SharedPrefs.getString(this@PatientDocumentsActivity, "sessionid")!!,username!!,usertype).enqueue(object : Callback<GetPatientDocumentsResponse> {
            ...
            override fun onResponse(call: Call<GetPatientDocumentsResponse>, response: Response<GetPatientDocumentsResponse>) {
                documents_progress_bar.visibility = View.GONE
             try{   if (response.isSuccessful && response.body() != null && response.body()!!.statuscode == 200 && response.body()!!.data.documents != null) {
                    ...
                    for (i in 0 until response.body()!!.data.documents!!.size) {
                        ...
                        if (response.body()!!.data.documents!!.get(i).doc_type.toLowerCase().contains("lab_report")) {
                            labReportsList.add(response.body()!!.data.documents!!.get(i))
                        } else if (response.body()!!.data.documents!!.get(i).doc_type.toLowerCase().contains("x_ray")) {
                            xRayList.add(response.body()!!.data.documents!!.get(i))
                        } else if (response.body()!!.data.documents!!.get(i).doc_type.toLowerCase().contains("doc_notes")) {
